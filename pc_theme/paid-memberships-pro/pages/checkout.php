<?php
	...
	{
		wp_redirect(get_site_url().'/membership-account',301);
		exit;
	}
	/**
...
<form id="pmpro_form" class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
	...
	<div id="pmpro_user_fields" class="pmpro_checkout">
		...
		<div class="pmpro_checkout-fields">
			...
			<div class="pmpro_checkout-field pmpro_checkout-field-bemail">
				...
				<input id="bemail" name="bemail" type="<?php echo ($pmpro_email_field_type ? 'email' : 'text'); ?>" class="input <?php echo pmpro_getClassForField("bemail");?>" size="30" value="<?php echo esc_attr($bemail); ?>" disabled />
	...
	<?php } elseif($current_user->ID && !$pmpro_review) { ?>
		<div id="pmpro_account_loggedin" class="pmpro_message pmpro_alert" style='clear: both;'>
	...
	<div id="pmpro_billing_address_fields" class="pmpro_checkout" <?php if(!$pmpro_requirebilling || apply_filters("pmpro_hide_billing_address_fields", false) ){ ?>style="display: none;"<?php } ?>>
		...
		<div class="pmpro_checkout-fields">
			...
			<div class="pmpro_checkout-field pmpro_checkout-field-bfirstname">
				...
				<input id="bfirstname" name="bfirstname" type="text" class="input <?php echo pmpro_getClassForField("bfirstname");?>" size="30" placeholder="<?php _e('First Name', 'paid-memberships-pro' );?>" value="<?php echo esc_attr($bfirstname); ?>" /></label>
			</div> <!-- end pmpro_checkout-field-bfirstname -->
			<div class="pmpro_checkout-field pmpro_checkout-field-blastname">
				<label for="blastname">
				<input id="blastname" name="blastname" type="text" class="input <?php echo pmpro_getClassForField("blastname");?>" size="30" placeholder='<?php _e('Last Name', 'paid-memberships-pro' );?>' value="<?php echo esc_attr($blastname); ?>" /></label>
			</div> <!-- end pmpro_checkout-field-blastname -->
			<div class="pmpro_checkout-field pmpro_checkout-field-baddress1">
				<label for="baddress1">
				<input id="baddress1" name="baddress1" type="text" class="input <?php echo pmpro_getClassForField("baddress1");?>" size="30" placeholder='<?php _e('Address 1', 'paid-memberships-pro' );?>' value="<?php echo esc_attr($baddress1); ?>" /></label>
			</div> <!-- end pmpro_checkout-field-baddress1 -->
			<div class="pmpro_checkout-field pmpro_checkout-field-baddress2">
				<label for="baddress2">
				<input id="baddress2" name="baddress2" type="text" class="input <?php echo pmpro_getClassForField("baddress2");?>" size="30" placeholder='<?php _e('Address 2', 'paid-memberships-pro' );?>' value="<?php echo esc_attr($baddress2); ?>" /></label>
			...
			<?php
				...
				if($longform_address) { ?>
					<div class="pmpro_checkout-field pmpro_checkout-field-bcity">
						<label for="bcity">
						<input id="bcity" name="bcity" type="text" class="input <?php echo pmpro_getClassForField("bcity");?>" size="30" placeholder='<?php _e('City', 'paid-memberships-pro' );?>' value="<?php echo esc_attr($bcity); ?>" /></label>
					...
					<div class="pmpro_checkout-field pmpro_checkout-field-bstate">
						<label for="bstate">
						<input id="bstate" name="bstate" type="text" class="input <?php echo pmpro_getClassForField("bstate");?>" size="30" placeholder='<?php _e('State', 'paid-memberships-pro' );?>' value="<?php echo esc_attr($bstate); ?>" /></label>
					</div> <!-- end pmpro_checkout-field-bstate -->
					<div class="pmpro_checkout-field pmpro_checkout-field-bzipcode">
						<label for="bzipcode">
						<input id="bzipcode" name="bzipcode" type="text" class="input <?php echo pmpro_getClassForField("bzipcode");?>" size="30" placeholder='<?php _e('Postal Code', 'paid-memberships-pro' );?>' value="<?php echo esc_attr($bzipcode); ?>" /></label>
				...
				<?php } else { ?>
					<div class="pmpro_checkout-field pmpro_checkout-field-bcity_state_zip">
						...
						<input id="bcity" name="bcity" type="text" class="input <?php echo pmpro_getClassForField("bcity");?>" size="14" placeholder='<?php _e('City, State Zip', 'paid-memberships-pro' );?>' value="<?php echo esc_attr($bcity); ?>" /></label>,
						<?php
			...
			<div class="pmpro_checkout-field pmpro_checkout-field-bemail">
				<label for="bemail">
				<input id="bemail" name="bemail" type="<?php echo ($pmpro_email_field_type ? 'email' : 'text'); ?>" class="input <?php echo pmpro_getClassForField("bemail");?>" size="30" placeholder='<?php _e('E-mail Address', 'paid-memberships-pro' );?>'value="<?php echo esc_attr($bemail); ?>" disabled /></label>
			</div> <!-- end pmpro_checkout-field-bemail -->
			<?php
				$pmpro_checkout_confirm_email = apply_filters("pmpro_checkout_confirm_email", false);
				if($pmpro_checkout_confirm_email) { ?>
					<div class="pmpro_checkout-field pmpro_checkout-field-bconfirmemail">
						...
						<input id="bconfirmemail" name="bconfirmemail" type="<?php echo ($pmpro_email_field_type ? 'email' : 'text'); ?>" class="input <?php echo pmpro_getClassForField("bconfirmemail");?>" size="30" placeholder='<?php _e('Confirm E-mail', 'paid-memberships-pro' );?>' value="<?php echo esc_attr($bconfirmemail); ?>" /></label>
					</div> <!-- end pmpro_checkout-field-bconfirmemail -->
				<?php } else { ?>
					<input type="hidden" name="bconfirmemail_copy" value="1" />
				<?php } ?>
		...
		<div id="pmpro_payment_information_fields" class="pmpro_checkout" <?php if(!$pmpro_requirebilling || apply_filters("pmpro_hide_payment_information_fields", false) ) { ?>style="display: none;"<?php } ?>>
			...
			<div class="pmpro_checkout-fields">
				...
				<div class="pmpro_checkout-field pmpro_payment-account-number">
					...
					<input id="AccountNumber" name="AccountNumber" class="input <?php echo pmpro_getClassForField("AccountNumber");?>" type="text" size="30" placeholder='<?php _e('Card Number', 'paid-memberships-pro' );?>' value="<?php echo esc_attr($AccountNumber); ?>" data-encrypted-name="number" autocomplete="off" /></label>
				</div>
				</div>
				<div class='col-md-6'>
					<div class="pmpro_checkout-field pmpro_payment-expiration">
						<label for="ExpirationMonth"><?php // _e('Expiration Date', 'paid-memberships-pro' );?>
						...
						</select><p> / </p><select id="ExpirationYear" name="ExpirationYear" class=" <?php echo pmpro_getClassForField("ExpirationYear");?>">
							<?php
				...
				<?php
					...
					<div class="col-md-6 col-md-pull-6">
						<div class="pmpro_checkout-field pmpro_payment-cvv">
							...
							<input id="CVV" name="CVV" type="text" size="4" placeholder='<?php _e('CVC', 'paid-memberships-pro')?>' value="<?php if(!empty($_REQUEST['CVV'])) { echo esc_attr($_REQUEST['CVV']); }?>" class="input <?php echo pmpro_getClassForField("CVV");?>" />  <small>(<a href="javascript:void(0);" onclick="javascript:window.open('<?php echo pmpro_https_filter(PMPRO_URL); ?>/pages/popup-cvv.html','cvv','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=600, height=475');"><?php _e("what's this?", 'paid-memberships-pro' );?></a>)</small>
						</div>
					</div>
				<?php } ?>
				<?php if($pmpro_show_discount_code) { ?>
		...
		<?php } else { ?>
			...
			<?php
				...
				<span id="pmpro_submit_span">
					...
					<input type="submit" class="pmpro_btn pmpro_btn-submit-checkout" id="actual_button" value="<?php if($pmpro_requirebilling) { _e('Submit and Check Out', 'paid-memberships-pro' ); } else { _e('Submit and Confirm', 'paid-memberships-pro' );}?> &raquo;" style="display:none;" />
					
					<!-- Custom button -->
					<input type="button" class="pmpro_btn pmpro_btn-submit-checkout" id="custom_button" value="<?php if($pmpro_requirebilling) { _e('Submit and Check Out', 'paid-memberships-pro' ); } else { _e('Submit and Confirm', 'paid-memberships-pro' );}?> &raquo;" />
				</span>
				<?php
				}
			?>
...
<script>
	window.onload = function(){
		...
		jQuery('#custom_button').click(function(){
			jQuery("#AccountNumber").attr('maxlength','19');
			jQuery("#AccountNumber").addClass('required');
			jQuery("#CVV").addClass('required');
			if(form.valid())
			{
				$('#actual_button').trigger('click');
			}
		});
		...
		/* jQuery('.pmpro_required').after('<span class="pmpro_asterisk requiredred"> <abbr title="Required Field">*</abbr></span>'); */
		jQuery('.pmpro_required').closest('.pmpro_checkout-field').find('label').append(' <font color="red">*</font>');
