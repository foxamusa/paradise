...
{
	jQuery('p.tml-user-pass1-wrap').find('label').html('<label for="pass1">Create Your Password <span class="requiredred">*</span></label>');
	jQuery('p.tml-user-pass2-wrap').find('label').html('<label for="pass2">Confirm Your Password <span class="requiredred">*</span></label>');
});
</script>
...
<div class="tml tml-register" id="theme-my-login<?php $template->the_instance(); ?>">
	...
	<form class="tmlregistrationform" name="registerform" id="registerform<?php $template->the_instance(); ?>" action="<?php $template->the_action_url( 'register', 'login_post' ); ?>" method="post">
		...
		<p class="tml-user-login-wrap">
			<label for="user_login<?php $template->the_instance(); ?>"><?php _e( 'Username', 'theme-my-login' ); ?> <span class="requiredred">*</span></label> 
			<input type="text" name="user_login" id="user_login<?php $template->the_instance(); ?>" class="input" value="<?php $template->the_posted_value( 'user_login' ); ?>" size="20" />
		</p>
		<?php endif; ?>
		
		<p class="tml-first-name-wrap">
			<label for="first_name<?php $template->the_instance(); ?>"><?php _e( 'First Name', 'theme-my-login' ); ?> <span class="requiredred">*</span></label> 
		...
		<p class="tml-last-name-wrap">
			<label for="last_name<?php $template->the_instance(); ?>"><?php _e( 'Last Name', 'theme-my-login' ); ?> <span class="requiredred">*</span></label> 
		...
		<p class="tml-phone-wrap">
			<label for="phone<?php $template->the_instance(); ?>"><?php _e( 'Phone Number', 'theme-my-login' ); ?> <span class="requiredred">*</span></label> 
		...
		<p class="tml-age-wrap">
			<label for="age<?php $template->the_instance(); ?>"><?php _e( 'Age (Optional)', 'theme-my-login' ); ?></label> 
			<input type="text" name="age" id="age<?php $template->the_instance(); ?>" class="input" value="<?php $template->the_posted_value( 'age' ); ?>" size="20" />
		</p>
		
		<p class="tml-user-email-wrap">
			<label for="user_email<?php $template->the_instance(); ?>"><?php _e( 'E-mail', 'theme-my-login' ); ?> <span class="requiredred">*</span></label> 
