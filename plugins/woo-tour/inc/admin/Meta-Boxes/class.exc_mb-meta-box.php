...
class EXC_MB_Meta_Box {
	...
	function show() { ?>
		...
		<input type="hidden" name="wp_exmeta_box_nonce" value="<?php esc_attr_e( wp_create_nonce( basename(__FILE__) ) ); ?>" />
	...
	function save( $post_id = 0 ) {
		...
		// Verify nonce
		if ( ! isset( $_POST['wp_exmeta_box_nonce'] ) || ! wp_verify_nonce( $_POST['wp_exmeta_box_nonce'], basename( __FILE__ ) ) )
			return $post_id;
