<?php
function parse_wt_table_func($atts, $content){
	global $style;
	...
	$args = woo_tour_query($posttype, $posts_per_page, $order, $orderby, $cat, $tag, $ids,$paged,$data_qr,$location,$meta_key, $meta_value);
	ob_start();
	$the_query = new WP_Query( $args );
	if($the_query->have_posts()){?>
		<div class="wt-table-lisst <?php echo 'table-style-'.esc_attr($style);?>" id="table-<?php echo $ID;?>">
			<table class="wt-table">
            	<?php if($style!='2'){?>
                <thead class="thead-inverse">
                  <tr>
                    <th><?php echo esc_html__("Image", "woo-tour");?></th>
                    <th>
						<?php echo esc_html__("Name", "woo-tour");?>
                    	<span class="wt-hidden-screen"><?php echo esc_html__("Details", "woo-tour");?></span>
                    </th>
                    <th class="wt-mb-hide"><?php echo esc_html__("Location", "woo-tour");?></th>
                    <th class="wt-mb-hide"><?php echo esc_html__("Price", "woo-tour");?></th>
                    <th class="wt-mb-hide"><?php echo esc_html__("Duration", "woo-tour");?></th>
                  </tr>
                </thead>
                <?php }?>
                <tbody>
                ...
                while($the_query->have_posts()){ $the_query->the_post();
                    ...
                    //include wt_get_plugin_url().'shortcode/content/content-table.php';
		...
		echo '<p class="woocommerce-info">'.esc_html__('No Tour Found','woo-tour').'</p>';
	}
	wp_reset_postdata();
...
add_shortcode( 'wt_table', 'parse_wt_table_func' );
add_action( 'after_setup_theme', 'wt_reg_vc' );
function wt_reg_vc(){
	if(function_exists('vc_map')){
	vc_map( array(
	   "name" => esc_html__("WooTours - Table", "woo-tour"),
	   "base" => "wt_table",
	   "class" => "",
	   "icon" => "icon-table",
	   "controls" => "full",
	   ...
	   "params" => array(
			 ...
			 "value" => array(
				...
				esc_html__('Classic', 'woo-tour') => '1',
				esc_html__('Modern', 'woo-tour') => '2',
			 ),
			 "description" => ''
		  ),	
		  array(
